 ...
 *
 * CONFIGURATION: set these constants in wp-config.php (or here):
 *   define('UGC_VERCEL_URL',   'https://ugc-scroll2.vercel.app');
 *   define('UGC_SYNC_SECRET',  'your-secret-string-matching-SYNC_SECRET-env-var');
 *
 * ACF FIELDS – adjust to match your actual field names/keys in ACF:
 *   UGC_ACF_PREMIUM_FIELD  – true/false checkbox for premium status
 *   UGC_ACF_AGE_FIELD      – number field for creator's age
 *   UGC_ACF_CITY_FIELD     – text field for creator's city
 *   UGC_ACF_BIO_FIELD      – textarea field for creator's bio
 *
...
define('UGC_ACF_AGE_FIELD',    'ika');
define('UGC_ACF_CITY_FIELD',   'kaupunki');
define('UGC_ACF_BIO_FIELD',    'lyhyt_kuvaus');
...
function ugc_sync_creator_to_vercel(int $post_id, WP_Post $post): void {
    ...
    // Read ACF fields — all optional, fall back to null gracefully
    $is_premium = false;
    $age        = null;
    $city       = null;
    $bio        = null;
    ...
    if (function_exists('get_field')) {
        ...
        $age_raw    = get_field(UGC_ACF_AGE_FIELD,    $post_id);
        $age        = $age_raw ? (int) $age_raw : null;
        $city_raw   = get_field(UGC_ACF_CITY_FIELD,   $post_id);
        $city       = $city_raw ? (string) $city_raw : null;
        $bio_raw    = get_field(UGC_ACF_BIO_FIELD,    $post_id);
